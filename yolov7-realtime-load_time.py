...
Bu kod .pt den onnx e dönüştürülmüş bir model çıktısının ekran kartındaki yükünü ve modelin yüklenme zamanını okumak amacıyla yazılmıştır.
This code was written to read the load on the video card of a model output converted from .pt to onnx and the load time of the model.
"""
...
model_name = ["yolov7.onnx","best.onnx"]

def model_load(model_name):
   
   
    model_load_start=time.time()
    model= YOLOv7(f"{model_name}", conf_thres=0.5, iou_thres=0.5, official_nms=True)
    ...
    if ret:
          if status:
               ...
               if p%2==0:
                   ...
                   model=model_load(model_name[0])
               else:
                   model=model_load(model_name[1])
          status=sayac(start)
